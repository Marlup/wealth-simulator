// bokeh_plot.js — rewritten for BokehJS 3.x (module-based)

import * as Bokeh from "https://cdn.bokeh.org/bokeh/release/bokeh-3.3.2.min.mjs";

const {
  figure, show, ColumnDataSource, Slider, Select,
  row, column
} = Bokeh;

const MONTHS_IN_YEAR = 12;

const source_balances = new ColumnDataSource({ data: { month: [], year: [], balance: [] } });
const source_earnings = new ColumnDataSource({ data: { yield_term: [], earning: [] } });
...
const s_principal = new Slider({ title: "Initial Amount (€)", start: 0, end: 100000, step: 1000, value: 10000 });
const s_roi = new Slider({ title: "Annual ROI", start: 0.0, end: 0.15, step: 0.005, value: 0.05 });
const s_duration = new Slider({ title: "Investment Duration (Years)", start: 1, end: 60, step: 1, value: 40 });
const s_frequency = new Slider({ title: "Payment Frequency (Months)", start: 1, end: 12, step: 1, value: 12 });
const s_contribution = new Slider({ title: "Annual Contribution (€)", start: 0, end: 50000, step: 500, value: 1200 });
const s_inc_contribution = new Slider({ title: "Increment Contribution Rate", start: 0.0, end: 1.0, step: 0.01, value: 0.01 });
const s_inflation = new Slider({ title: "Inflation Rate", start: 0.0, end: 1.0, step: 0.01, value: 0.02 });
const s_retirement_income = new Slider({ title: "Monthly Retirement Income (€)", start: 0, end: 5000, step: 100, value: 500 });
const s_retirement_at = new Slider({ title: "Retirement At (Years)", start: 0, end: 60, step: 1, value: 30 });
const select_tax = new Select({ title: "Tax (Fixed Rate)", value: "0.21", options: ["0.15", "0.21", "0.30"] });
...
const layout = row(controls, column(plot_e, plot_b));
show(layout, document.getElementById("bokeh-container"));
